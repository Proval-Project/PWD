namespace EstimateRequestSystem.DTOs
{
    ...
    public class BodySaveSpecificationDto
    {
        ...
        public string RatingUnit { get; set; } = string.Empty; // RatingUnit 추가
    ...
    public class AccessorySpecificationDto
    {
        ...
        public string? PosMakerCode { get; set; } // 포지셔너 메이커 코드 (CHAR(10))
        
        public string? SolCode { get; set; }        // 솔레노이드 모델 코드 (CHAR(1))
        public string? SolMakerCode { get; set; } // 솔레노이드 메이커 코드 (CHAR(10))
        
        public string? LimCode { get; set; }        // 리미터 모델 코드 (CHAR(1))
        public string? LimMakerCode { get; set; } // 리미터 메이커 코드 (CHAR(10))
        
        public string? ASCode { get; set; }         // 에어 서플라이 모델 코드 (CHAR(1))
        public string? ASMakerCode { get; set; } // 에어 서플라이 메이커 코드 (CHAR(10))
        
        public string? VolCode { get; set; }        // 볼륨 부스터 모델 코드 (CHAR(1))
        public string? VolMakerCode { get; set; } // 볼륨 부스터 메이커 코드 (CHAR(10))
        
        public string? AirOpCode { get; set; }      // 에어 오퍼레이터 모델 코드 (CHAR(1))
        public string? AirOpMakerCode { get; set; } // 에어 오퍼레이터 메이커 코드 (CHAR(10))
        
        public string? LockupCode { get; set; }     // 락업 모델 코드 (CHAR(1))
        public string? LockupMakerCode { get; set; } // 락업 메이커 코드 (CHAR(10))
        
        public string? SnapActCode { get; set; }    // 스냅 액션 모델 코드 (CHAR(1))
        public string? SnapActMakerCode { get; set; } // 스냅 액션 메이커 코드 (CHAR(10))
    }
