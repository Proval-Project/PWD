...
namespace EstimateRequestSystem.Models
{
    public class DataSheetLv3
    {
        ...
        public string? TrimPortSizeUnit { get; set; }
        ...
        [Column("PosMakerCode")]
        public string? PosMakerCode { get; set; } // New column for Positioner AccMakerCode (CHAR(1))
        [Column("SolCode")]
        public string? SolCode { get; set; }
        [Column("SolMakerCode")]
        public string? SolMakerCode { get; set; } // New column for Solenoid AccMakerCode (CHAR(1))
        [Column("LimCode")]
        public string? LimCode { get; set; }
        [Column("LimMakerCode")]
        public string? LimMakerCode { get; set; } // New column for Limit Switch AccMakerCode (CHAR(1))
        [Column("ASCode")]
        public string? ASCode { get; set; }
        [Column("ASMakerCode")]
        public string? ASMakerCode { get; set; } // New column for Air Set AccMakerCode (CHAR(1))
        [Column("VolCode")]
        public string? VolCode { get; set; }
        [Column("VolMakerCode")]
        public string? VolMakerCode { get; set; } // New column for Volume Booster AccMakerCode (CHAR(1))
        [Column("AirOpCode")]
        public string? AirOpCode { get; set; }
        [Column("AirOpMakerCode")]
        public string? AirOpMakerCode { get; set; } // New column for Air Operated AccMakerCode (CHAR(1))
        [Column("LockupCode")]
        public string? LockupCode { get; set; }
        [Column("LockupMakerCode")]
        public string? LockupMakerCode { get; set; } // New column for Lock Up AccMakerCode (CHAR(1))
        [Column("SnapActCode")]
        public string? SnapActCode { get; set; }
        [Column("SnapActMakerCode")]
        public string? SnapActMakerCode { get; set; } // New column for Snap Acting Relay AccMakerCode (CHAR(1))

        // Navigation properties
        public virtual EstimateSheetLv1 EstimateSheet { get; set; } = null!;
        // 악세사리, Act 관련 Navigation Properties 제거됨 - 프론트엔드에서 관리
    }
}
